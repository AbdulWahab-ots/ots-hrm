...
import { EmployeePayload, GetHolidaysPayload } from "@/utils/types";
import { FormikHelpers } from "formik";
import { setIsLoading } from "@/store/features/global/globalSlice";
import { useRouter } from "next/navigation";
import { PiBugBeetle } from "react-icons/pi";
import { getAllVacationsAPI, getLeaveStatsAPI } from "@/services/employeeService";
import { GetVacationsPayload } from "@/utils/company";
...
const LeaveDashboard: React.FC = () => {
  ...
  // Real alerts, replacing the previous hardcoded 5-item list. Each is only
  // shown when its count is non-zero - an "alert" for a count of zero isn't
  // actionable. No per-alert "time ago" is shown since these are point-in-time
  // aggregate counts, not discrete timestamped events.
  useEffect(() => {
    const fetchAlerts = async () => {
      ...
      try {
        const [leaveStats, leaveTypesRes, allLeaveVacationsRes, holidaysRes] =
          await Promise.all([
            getLeaveStatsAPI(dispatch),
            fetchAllLeaveTypes(dispatch),
            getAllVacationsAPI(dispatch, {
            ...
            } as GetVacationsPayload),
            getAllHolidaysAPI(dispatch, {
              pagedListRequest: { pageNo: 1, pageSize: 1000, getAllRecords: true },
              queryOptionsRequest: { filtersRequest: [], sortRequest: [], includes: [] },
            } as GetHolidaysPayload),
          ]);

        const newAlerts: Alert[] = [];

        // "Employees exceeding leave limit": sum each employee's approved/pending
        // LEAVE days this year per leave type, compare against that type's
        // maxDaysPerYear.
        const leaveTypes = leaveTypesRes?.result?.data ?? [];
        const currentYear = today.getFullYear();
        const usedDaysByEmployeeAndType: Record<string, number> = {};
        (allLeaveVacationsRes?.result?.data ?? []).forEach((vacation: any) => {
          if (
            (vacation.status === "APPROVED" || vacation.status === "PENDING") &&
            new Date(vacation.createdAt).getFullYear() === currentYear &&
            vacation.requestedBy &&
            vacation.typeId
          ) {
            const key = `${vacation.requestedBy}::${vacation.typeId}`;
            usedDaysByEmployeeAndType[key] =
              (usedDaysByEmployeeAndType[key] || 0) + (Number(vacation.totalDays) || 0);
          }
        });
        const maxDaysByType: Record<string, number> = {};
        leaveTypes.forEach((lt: any) => {
          maxDaysByType[lt.id] = lt.maxDaysPerYear ?? Infinity;
        });
        const employeesExceeding = new Set<string>();
        Object.entries(usedDaysByEmployeeAndType).forEach(([key, used]) => {
          const [employeeId, typeId] = key.split("::");
          if (used > (maxDaysByType[typeId] ?? Infinity)) {
            employeesExceeding.add(employeeId);
          }
        });
        if (employeesExceeding.size > 0) {
          newAlerts.push({
            id: 1,
            bg: "var(--g-red-100)",
            icon: <PiBugBeetle size={35} className="text-g-gray-900 font-normal" />,
            text: `${employeesExceeding.size} Employee${employeesExceeding.size === 1 ? "" : "s"} exceeding leave limit`,
          });
        }

        // "Holidays are nearing": any holiday with a date in the next 30 days.
        const nearingHolidays = (holidaysRes?.result?.data ?? []).filter((holiday) =>
          (holiday.dates ?? []).some((dateStr) => {
            const date = new Date(dateStr);
            return date >= today && date <= upcoming;
          })
        );
        if (nearingHolidays.length > 0) {
          newAlerts.push({
            id: 2,
            bg: "var(--g-blue-100)",
            icon: holidayIcon,
            text:
              nearingHolidays.length === 1
                ? `${nearingHolidays[0].name} is nearing`
                : `${nearingHolidays.length} Holidays are nearing`,
          });
        }

        // "Leaves need action": pending leave requests company-wide.
        const pendingLeaves = leaveStats?.result?.pending ?? 0;
        if (pendingLeaves > 0) {
          newAlerts.push({
            id: 3,
            bg: "var(--g-red-100)",
            icon: <PiBugBeetle size={35} className="text-g-gray-900 font-normal" />,
            text: `${pendingLeaves} leave${pendingLeaves === 1 ? "" : "s"} need action`,
          });
        }
      ...
      <div className="bg-g-background-100 p-4 lg:p-6 border-[1px] border-g-gray-alpha-400 rounded-[var(--g-radius-md)] shadow-geist-card">
        ...
        <div className="mt-4 space-y-4">
          {visibleAlerts.length === 0 ? (
            <p className="text-sm text-g-gray-800">No alerts right now.</p>
          ) : (
